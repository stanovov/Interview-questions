...
+ #### Отложенное выполнение:
  ...
  программы лямбда-выражение и затем можем его вызвать при необходимости неопрделененное количество раз в различных частях
  программы.
...
Функциональный интерфейса - это интерфейс, который содержит только 1 абстрактный метод.
...
результатов. По моей логие это все из этого списка: `Consumer<T>`, `Function<T,R>`, `Predicate<T>`, `Supplier<T>`, 
...
Шаблон предлагает разбить процесс конструирования объекта на отдельные шаги (например для нашего класса - присвоитьИмя, 
присвоитьВозраст и т.д.). Чтобы создавать объекты таким образом, нам нужно поочередно вызывать методы строителя. При 
этом нам не нужно запускать все шаги, а только те, которые нужны для производства объекта определенной конфигурации.
